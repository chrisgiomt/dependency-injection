import sqlite3
import os
from schemas import Class, Department, Enrollment, Dropped, User
...
#Remove users_database if it exists before creating and populating it
...
#Hashes a password using the method described in Simon Willison’s Password hashing in Python with pbkdf2.
...
sample_users = []
for n in name:
    username = n.replace(" ", "").lower()

    #Assign password via username (takes long as shit)
...
def populate_database():
    ...
    users_table = """CREATE TABLE IF NOT EXISTS users (
                    ...
                    role TEXT CHECK (role IN ('Instructor', 'Student', 'Registrar'))
                )"""
